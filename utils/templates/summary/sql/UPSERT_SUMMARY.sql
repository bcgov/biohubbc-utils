...
do $ $ declare _summary_template_id integer;

-- set after template is created
...
_taxonomy_ids integer [] := array [1,2,3];

-- wild taxonomic IDs from elastic search
_taxonomy_id integer;

-- used as an index in the loop, do not set
...
WHERE
	"name" = _template_name
	AND "version" = _template_version;

FOREACH _taxonomy_id IN ARRAY _taxonomy_ids LOOP
INSERT INTO
	summary_template_species (
...
WHERE
	...
	AND summary_template_species.summary_template_id = _summary_template_id;
