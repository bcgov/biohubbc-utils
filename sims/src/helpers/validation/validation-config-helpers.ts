/*
 * Common validation rules, that can be re-used in any validation config.
 *
 * Note: Do not alter these validation rules unless you are certain that the changes apply to any validation configs
  ...
  return [
    {
      column_format_validator: {
        reg_exp: '^([7-9]|1[0-1])$',
        reg_exp_flags: 'g',
        expected_format: 'UTM Zone must be a number between 7 and 11.'
      }
    }
  ];
};
...
export const eastingValidator = () => {
  return [
    {
      column_format_validator: {
        reg_exp: '^[1-9][0-9][0-9][0-9][0-9][0-9](?:\\.\\d+)?$',
        reg_exp_flags: 'g',
...
export const northingValidator = () => {
  return [
    {
      column_format_validator: {
        reg_exp: '^[1-9][0-9][0-9][0-9][0-9][0-9][0-9](?:\\.\\d+)?$',
        reg_exp_flags: 'g',
