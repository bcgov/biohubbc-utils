...
export interface TemplateBuilderConfig {
  ...
  /**
   * Details about the the transformation config.
   */
  transform?: {
    /**
     * The transformation builder object.
     */
    inputBuilderObject: Record<string, any>;
  };
  /**
...
export class TemplateBuilder {
  ...
  async build(): Promise<void> {
    try {
      ...
      this.transformString = await this._buildTransformationConfig();
   ...
   * @return {*}  {Promise<string>}
   */
  async _buildTransformationConfig(): Promise<string> {
  ...
  async _buildSQLUpsert(): Promise<void> {
    ...
    const operations = [
      ...
      {
        replaceValue: this.transformString || '',
        replaceToken: '___TRANSFORMATION_CONFIG___'
      }
    ];
